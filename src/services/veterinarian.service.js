  ...
  try {
    ...
    if (existingVet)
      throw new ServiceError(
        409,
        "Ya existe un veterinario asociado a este usuario"
      );

    const newVeterinarian = await veterinarian_repository.save(
      veterinarianData,
      t
    );
...
export const findVeterinarianById = async (id) => {
  try {
    const veterinarian = await veterinarian_repository.findById(id);
    if (!veterinarian) {
      throw new ServiceError(404, "Veterinario no encontrado");
    }
    return veterinarian;
  } catch (e) {
    if (e instanceof ValidationError) {
